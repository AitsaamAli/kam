...
Per head-search's direction and strategy-director's sequencing: **this is not another 90-100 topic batch.** Three of those have already shipped in the last month with zero measurement of what worked. This plan is small (2-4 pieces), self-limiting, and explicitly scoped to the one gap multiple specialists actually confirmed — not a return to volume-first production.
...
Internal links from the two highest-traffic-adjacent existing pages this content is most relevant to: the `/pricing` page and the Section H trust pages (`red-flags-fake-tax-consultant-pakistan`, `fbr-certified-vs-non-certified-consultant-pakistan`) — a visitor comparing consultants is likely already reading one of those. No paid distribution — consistent with the near-zero-budget constraint.
...
- `brief-writer` for the per-piece execution brief (target queries, H2 outline, the exact comparison-table structure, internal links) — this plan sets direction, not the line-by-line brief.
- `comparison-pages` (GEO department) for the actual page format/schema.

---
**Verified:** the competitor facts used here trace directly to competitor-intel's live-checked findings from earlier this session.
**Assumption:** "60-90 days" as the retest window is a reasoned estimate for how long a new page takes to surface in AI answers, not a guaranteed timeline — KNOWLEDGE.md notes real, sustained results generally take months for a new/lower-authority page, and this project has no confirmed authority baseline.
