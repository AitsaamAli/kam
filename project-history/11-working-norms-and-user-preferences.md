...
## 11a. Never claim "SECP Registered" or "ISO 9001:2015 certified" as Kamboh Associates' own credentials
Per explicit user instruction (2026-09-04): the site does not hold SECP registration or ISO 9001:2015 certification as a credential of the firm itself, so no page may claim either as a self-credential (badges, ticker text, accreditation grids, footer badges, JSON-LD `hasCredential`, "About the Firm" boilerplate sentences, FAQ Q&As built to justify the credential, etc.). This is distinct from — and must not remove — legitimate SECP mentions: SECP Company Registration as a named service/fee-schedule line item, blog content about the SECP registration *process*, or general/topical references to SECP-registered companies (client companies, other businesses, industry advice). When auditing, read full sentence context before touching any "SECP" mention; every ISO 9001/ISO-9001 mention found sitewide, by contrast, was a self-claim with no legitimate alternative use, so ISO removal did not need the same scoping. `blogs/why-lahore-businesses-choose-kamboh-associates.html` needed a structural rewrite (two dedicated H2 sections, a credentials table, and matching visible-FAQ + JSON-LD FAQ entries were built specifically to justify these two credentials) rather than simple phrase deletion — see commit `eae6a515`.
