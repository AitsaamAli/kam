const fs = require('fs');
const path = require('path');
const BLOGS_DIR = path.join(__dirname, 'blogs');

const LOCATIONS = [
  // ── DHA PHASES ──────────────────────────────────────
  { slug: 'tax-consultant-dha-phase-3-lahore', area: 'DHA Phase 3', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-phase-4-lahore', area: 'DHA Phase 4', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-phase-7-lahore', area: 'DHA Phase 7', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-phase-8-lahore', area: 'DHA Phase 8', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-phase-9-lahore', area: 'DHA Phase 9', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-rahbar-lahore', area: 'DHA Rahbar', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-dha-raya-lahore', area: 'DHA Raya', city: 'Lahore', province: 'Punjab' },

  // ── ASKARI PHASES ────────────────────────────────────
  { slug: 'tax-consultant-askari-1-lahore', area: 'Askari 1', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-askari-10-lahore', area: 'Askari 10', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-askari-11-lahore', area: 'Askari 11', city: 'Lahore', province: 'Punjab' },

  // ── NEW HOUSING SOCIETIES ────────────────────────────
  { slug: 'tax-consultant-lake-city-lahore', area: 'Lake City', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-al-kabir-town-lahore', area: 'Al-Kabir Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-khayaban-e-amin-lahore', area: 'Khayaban-e-Amin', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-green-city-lahore', area: 'Green City', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-al-hamra-town-lahore', area: 'Al-Hamra Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-awan-town-lahore', area: 'Awan Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ali-town-lahore', area: 'Ali Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-minhaj-town-lahore', area: 'Minhaj Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-phase-3-lahore', area: 'Johar Town Phase 3', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-h1-lahore', area: 'Johar Town H1', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-h2-lahore', area: 'Johar Town H2', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-j1-lahore', area: 'Johar Town J1', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-j4-lahore', area: 'Johar Town J4', city: 'Lahore', province: 'Punjab' },

  // ── MODEL TOWN BLOCKS ────────────────────────────────
  { slug: 'tax-consultant-model-town-a-lahore', area: 'Model Town Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-model-town-b-lahore', area: 'Model Town Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-model-town-c-lahore', area: 'Model Town Block C', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-model-town-extension-lahore', area: 'Model Town Extension', city: 'Lahore', province: 'Punjab' },

  // ── MAJOR ROADS / MARKETS ────────────────────────────
  { slug: 'tax-consultant-mm-alam-road-lahore', area: 'M.M. Alam Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-main-boulevard-gulberg-lahore', area: 'Main Boulevard Gulberg', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-main-boulevard-lahore', area: 'Main Boulevard', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gt-road-lahore', area: 'GT Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-multan-road-lahore', area: 'Multan Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-college-road-lahore', area: 'College Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-link-road-lahore', area: 'Link Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ring-road-lahore', area: 'Ring Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-shalimar-link-road-lahore', area: 'Shalimar Link Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ghalib-market-lahore', area: 'Ghalib Market', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-packages-mall-lahore', area: 'Packages Mall Area', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-emporium-mall-lahore', area: 'Emporium Mall Area', city: 'Lahore', province: 'Punjab' },

  // ── TOWNSHIPS & COLONIES ─────────────────────────────
  { slug: 'tax-consultant-township-sector-b1-lahore', area: 'Township Sector B1', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-township-sector-c-lahore', area: 'Township Sector C', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-liaquat-colony-lahore', area: 'Liaquat Colony', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-saleem-town-lahore', area: 'Saleem Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ghosia-colony-lahore', area: 'Ghosia Colony', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulistan-colony-lahore', area: 'Gulistan Colony', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-muslim-park-lahore', area: 'Muslim Park', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-naseerabad-lahore', area: 'Naseerabad', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-shafiqabad-lahore', area: 'Shafiqabad', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-tajpura-lahore', area: 'Tajpura', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-hanjarwal-lahore', area: 'Hanjarwal', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-nishtar-town-lahore', area: 'Nishtar Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-kot-khawaja-saeed-lahore', area: 'Kot Khawaja Saeed', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gajumata-lahore', area: 'Gajumata', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sundar-industrial-lahore', area: 'Sundar Industrial Estate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-batapur-lahore', area: 'Batapur', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-kala-shah-kaku-lahore', area: 'Kala Shah Kaku', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-kot-abdul-malik-lahore', area: 'Kot Abdul Malik', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-raja-jang-lahore', area: 'Raja Jang', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-kamahan-lahore', area: 'Kamahan', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-iqbal-park-lahore', area: 'Iqbal Park', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-data-nagar-lahore', area: 'Data Nagar', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-shad-bagh-west-lahore', area: 'Shad Bagh West', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-chungi-amar-sadhu-lahore', area: 'Chungi Amar Sadhu', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-qaddafi-stadium-lahore', area: 'Qaddafi Stadium Area', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-garden-block-lahore', area: 'Garden Block', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sir-syed-town-lahore', area: 'Sir Syed Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-allama-iqbal-industrial-estate-lahore', area: 'Allama Iqbal Industrial Estate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-kot-lakhpat-industrial-lahore', area: 'Kot Lakhpat Industrial Area', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sundar-road-lahore', area: 'Sundar Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sharaqpur-lahore', area: 'Sharaqpur', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-pattoki-lahore', area: 'Pattoki', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-chunian-lahore', area: 'Chunian', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-muridke-lahore', area: 'Muridke', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ferozewala-lahore', area: 'Ferozewala', city: 'Lahore', province: 'Punjab' },

  // ── WALLED CITY / OLD LAHORE AREAS ──────────────────
  { slug: 'tax-consultant-delhi-gate-lahore', area: 'Delhi Gate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-lahori-gate-lahore', area: 'Lahori Gate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-lohari-gate-lahore', area: 'Lohari Gate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-akbari-gate-lahore', area: 'Akbari Gate', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-mcleod-road-lahore', area: 'McLeod Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-brandreth-road-lahore', area: 'Brandreth Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-hall-road-lahore', area: 'Hall Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-urdu-bazaar-lahore', area: 'Urdu Bazaar', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-shah-alam-market-lahore', area: 'Shah Alam Market', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-rang-mahal-lahore', area: 'Rang Mahal', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-timber-market-lahore', area: 'Timber Market', city: 'Lahore', province: 'Punjab' },

  // ── ALLAMA IQBAL TOWN BLOCKS ─────────────────────────
  { slug: 'tax-consultant-ait-block-a-lahore', area: 'AIT Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-b-lahore', area: 'AIT Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-c-lahore', area: 'AIT Block C', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-d-lahore', area: 'AIT Block D', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-e-lahore', area: 'AIT Block E', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-f-lahore', area: 'AIT Block F', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-g-lahore', area: 'AIT Block G', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-ait-block-h-lahore', area: 'AIT Block H', city: 'Lahore', province: 'Punjab' },

  // ── IQBAL TOWN BLOCKS ────────────────────────────────
  { slug: 'tax-consultant-iqbal-town-block-a-lahore', area: 'Iqbal Town Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-iqbal-town-block-b-lahore', area: 'Iqbal Town Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-iqbal-town-block-c-lahore', area: 'Iqbal Town Block C', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-iqbal-town-block-j-lahore', area: 'Iqbal Town Block J', city: 'Lahore', province: 'Punjab' },

  // ── SAMANABAD BLOCKS ─────────────────────────────────
  { slug: 'tax-consultant-samanabad-block-a-lahore', area: 'Samanabad Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-samanabad-block-b-lahore', area: 'Samanabad Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-samanabad-block-d-lahore', area: 'Samanabad Block D', city: 'Lahore', province: 'Punjab' },

  // ── GREEN TOWN BLOCKS ────────────────────────────────
  { slug: 'tax-consultant-green-town-block-a-lahore', area: 'Green Town Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-green-town-block-b-lahore', area: 'Green Town Block B', city: 'Lahore', province: 'Punjab' },

  // ── GARDEN TOWN BLOCKS ───────────────────────────────
  { slug: 'tax-consultant-garden-town-block-a-lahore', area: 'Garden Town Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-garden-town-block-b-lahore', area: 'Garden Town Block B', city: 'Lahore', province: 'Punjab' },

  // ── NEAR LANDMARKS ───────────────────────────────────
  { slug: 'tax-consultant-near-airport-lahore', area: 'Near Airport', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-high-court-lahore', area: 'Near High Court', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-lums-lahore', area: 'Near LUMS', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-pu-lahore', area: 'Near Punjab University', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-uet-lahore', area: 'Near UET', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-services-hospital-lahore', area: 'Near Services Hospital', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-near-mayo-hospital-lahore', area: 'Near Mayo Hospital', city: 'Lahore', province: 'Punjab' },

  // ── FAISAL TOWN BLOCKS ───────────────────────────────
  { slug: 'tax-consultant-faisal-town-block-a-lahore', area: 'Faisal Town Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-faisal-town-block-b-lahore', area: 'Faisal Town Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-faisal-town-block-c-lahore', area: 'Faisal Town Block C', city: 'Lahore', province: 'Punjab' },

  // ── MISC LAHORE LOCALITIES ───────────────────────────
  { slug: 'tax-consultant-banda-road-lahore', area: 'Banda Road', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-new-muslim-town-lahore', area: 'New Muslim Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulshan-e-lahore', area: 'Gulshan-e-Lahore', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-cavalry-ground-lahore', area: 'Cavalry Ground', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-nespak-housing-lahore', area: 'NESPAK Housing Society', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-wapda-town-phase-2-lahore', area: 'Wapda Town Phase 2', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-medical-housing-society-lahore', area: 'Medical Housing Society', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-engineers-town-lahore', area: "Engineers Town", city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-paragon-city-phase-2-lahore', area: 'Paragon City Phase 2', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-tulip-block-bahria-lahore', area: 'Tulip Block Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-jasmine-block-bahria-lahore', area: 'Jasmine Block Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sector-c-bahria-lahore', area: 'Sector C Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sector-e-bahria-lahore', area: 'Sector E Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-sector-f-bahria-lahore', area: 'Sector F Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-tauheed-block-bahria-lahore', area: 'Tauheed Block Bahria Town', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-punjab-small-industries-lahore', area: 'Punjab Small Industries Colony', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-airport-housing-society-lahore', area: 'Airport Housing Society', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-block-e1-lahore', area: 'Johar Town Block E1', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-johar-town-block-e2-lahore', area: 'Johar Town Block E2', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulshan-ravi-block-a-lahore', area: 'Gulshan-e-Ravi Block A', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulshan-ravi-block-b-lahore', area: 'Gulshan-e-Ravi Block B', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulshan-ravi-block-c-lahore', area: 'Gulshan-e-Ravi Block C', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-gulshan-ravi-block-d-lahore', area: 'Gulshan-e-Ravi Block D', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-eden-abad-lahore', area: 'Eden Abad', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-eden-city-lahore', area: 'Eden City', city: 'Lahore', province: 'Punjab' },
  { slug: 'tax-consultant-eden-motions-lahore', area: 'Eden Gardens (Bedian Road)', city: 'Lahore', province: 'Punjab' },
];

const SHARED_STYLE = `<style>:root{--navy:#0F172A;--teal:#14B8A6;--gold-light:#D4AF37;--font-display:'Cinzel',serif;--font-serif:'Cormorant Garamond',Georgia,serif;--font-body:'DM Sans',system-ui,sans-serif;--max-w:1200px}*{box-sizing:border-box;margin:0;padding:0}body{font-family:var(--font-body);background:#fff;color:#0F172A;line-height:1.65;overflow-x:hidden}a{text-decoration:none;color:inherit}ul{list-style:none}.container{max-width:var(--max-w);margin:0 auto;padding:0 24px}.topbar{background:#0F172A;padding:8px 0}.topbar .container{display:flex;justify-content:center;overflow:hidden}.topbar-promo-wrap{display:inline-flex;animation:kascroll 28s linear infinite}.topbar-promo{font-size:.73rem;color:rgba(148,163,184,.9);white-space:nowrap;flex-shrink:0}@keyframes kascroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}.navbar{position:sticky;top:0;z-index:1000;background:rgba(255,255,255,.97);backdrop-filter:blur(20px);border-bottom:1px solid #E2E8F0}.navbar .container{display:flex;align-items:center;justify-content:space-between;height:68px}.logo{display:flex;align-items:center;gap:12px}.logo-name{font-family:var(--font-display);font-size:.92rem;font-weight:700;letter-spacing:.07em;line-height:1.2}.logo-sub{font-size:.56rem;letter-spacing:.18em;color:var(--gold-light);text-transform:uppercase}.nav-links{display:flex;align-items:center;gap:4px}.nav-links>li{position:relative;list-style:none}.nav-links>li>a{font-size:.84rem;font-weight:500;color:#1E293B;display:flex;align-items:center;gap:4px;padding:8px 14px;border-radius:8px;transition:all .2s}.nav-links>li>a:hover{color:#14B8A6;background:#F0FDFA}.nav-chevron{font-size:.6rem;opacity:.6}.nav-drop{position:absolute;top:calc(100% + 8px);left:50%;transform:translateX(-50%) translateY(-6px);min-width:200px;background:#fff;border:1px solid #E2E8F0;border-radius:12px;box-shadow:0 8px 32px rgba(15,23,42,.12);padding:8px;opacity:0;visibility:hidden;pointer-events:none;transition:all .2s}.nav-links>li:hover .nav-drop{opacity:1;visibility:visible;pointer-events:auto;transform:translateX(-50%) translateY(0)}.nav-links>li:last-child .nav-drop{left:auto;right:0;transform:translateY(-6px)}.nav-links>li:last-child:hover .nav-drop{transform:translateY(0)}.nav-drop a{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:8px;font-size:.82rem;color:#1E293B;transition:all .15s}.nav-drop a:hover{background:#F0FDFA;color:#14B8A6}.nav-drop a i{width:16px;text-align:center;color:#14B8A6}.nav-drop-divider{height:1px;background:#E2E8F0;margin:4px 8px}.nav-actions{display:flex;align-items:center;gap:10px}.nav-phone{font-size:.8rem;color:#64748B;display:flex;align-items:center;gap:6px}.nav-phone i{color:#14B8A6}.nav-phone a{color:#1E293B;font-weight:500}.nav-cta{background:#0F172A;color:#fff!important;padding:8px 18px!important;border-radius:8px!important;font-size:.82rem!important;font-weight:600!important;transition:all .2s!important}.nav-cta:hover{background:#14B8A6!important}.hamburger{display:none;flex-direction:column;gap:5px;cursor:pointer;background:none;border:none;padding:4px}.hamburger span{width:22px;height:2px;background:#64748B;border-radius:2px;display:block}.mobile-nav{display:none;flex-direction:column;position:fixed;top:68px;left:0;right:0;z-index:999;background:#fff;border-bottom:3px solid #14B8A6;box-shadow:0 12px 40px rgba(0,0,0,.15)}.mobile-nav.open{display:flex}.mobile-nav a{padding:14px 24px;border-bottom:1px solid #F1F5F9;font-size:.9rem;color:#64748B}.blog-hero{background:linear-gradient(135deg,#0F172A 0%,#1E3A5F 60%,#0F4C5C 100%);padding:64px 0 52px;position:relative;overflow:hidden}.blog-hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 70% 30%,rgba(20,184,166,.1) 0%,transparent 60%)}.blog-hero .container{position:relative;z-index:1}.breadcrumb{display:flex;align-items:center;gap:8px;font-size:.78rem;color:rgba(148,163,184,.7);margin-bottom:20px;flex-wrap:wrap}.breadcrumb a{color:rgba(148,163,184,.7)}.breadcrumb a:hover{color:#14B8A6}.breadcrumb i{font-size:.55rem}.blog-cat{display:inline-flex;align-items:center;gap:5px;background:rgba(20,184,166,.15);border:1px solid rgba(20,184,166,.3);border-radius:100px;padding:4px 14px;font-size:.68rem;color:#14B8A6;font-family:var(--font-display);letter-spacing:.1em;text-transform:uppercase;margin-bottom:14px}.blog-hero h1{font-family:var(--font-serif);font-size:clamp(1.9rem,4vw,3rem);font-weight:700;color:#F1F5F9;line-height:1.15;margin-bottom:16px;max-width:800px}.blog-meta{display:flex;flex-wrap:wrap;gap:18px;font-size:.8rem;color:rgba(148,163,184,.8)}.blog-meta span{display:flex;align-items:center;gap:5px}.blog-meta i{color:#14B8A6}.blog-layout{display:grid;grid-template-columns:1fr 300px;gap:48px;padding:56px 0 80px;align-items:start}.article-body{background:#fff;border:1px solid #E2E8F0;border-radius:20px;padding:40px 44px;box-shadow:0 1px 3px rgba(15,23,42,.08)}.article-lead{font-family:var(--font-serif);font-size:1.15rem;color:#1E293B;line-height:1.8;margin-bottom:32px;padding-bottom:24px;border-bottom:1px solid #E2E8F0}.article-body h2{font-family:var(--font-serif);font-size:1.5rem;font-weight:600;color:#0F172A;margin:36px 0 14px;display:flex;align-items:center;gap:10px}.article-body h2::before{content:'';width:4px;height:24px;background:linear-gradient(135deg,#14B8A6,#0D9488);border-radius:2px;flex-shrink:0}.article-body p{font-size:.96rem;color:#1E293B;line-height:1.82;margin-bottom:16px}.article-body ul{margin:8px 0 16px 20px;display:flex;flex-direction:column;gap:6px}.article-body ul li{font-size:.94rem;color:#1E293B;list-style:disc;line-height:1.7}.article-body table{width:100%;border-collapse:collapse;margin:16px 0}.article-body th{background:#0F172A;color:#fff;padding:10px 14px;font-size:.84rem;text-align:left}.article-body td{padding:10px 14px;border-bottom:1px solid #E2E8F0;font-size:.88rem}.article-body tr:nth-child(even) td{background:#F8FAFC}.highlight-box{background:rgba(20,184,166,.07);border-left:4px solid #14B8A6;border-radius:0 12px 12px 0;padding:16px 20px;margin:20px 0}.highlight-box p{margin:0;font-size:.93rem}.faq-item{border:1px solid #E2E8F0;border-radius:12px;margin-bottom:10px;overflow:hidden}.faq-q{padding:15px 20px;font-weight:600;cursor:pointer;display:flex;justify-content:space-between;align-items:center;font-size:.92rem;background:#F8FAFC}.faq-q:hover{color:#14B8A6}.faq-q i{transition:transform .3s;color:#14B8A6}.faq-a{display:none;padding:14px 20px;border-top:1px solid #E2E8F0;font-size:.88rem;line-height:1.7}.faq-item.open .faq-a{display:block}.faq-item.open .faq-q i{transform:rotate(180deg)}.sidebar-card{background:#fff;border:1px solid #E2E8F0;border-radius:12px;padding:22px;margin-bottom:20px}.sidebar-card h4{font-family:var(--font-serif);font-size:1rem;font-weight:700;margin-bottom:12px;padding-bottom:10px;border-bottom:2px solid #E2E8F0}.consult-card{background:linear-gradient(135deg,#0F172A,#1E3A5F);border-radius:12px;padding:24px;color:#fff;text-align:center;margin-bottom:20px}.consult-card h4{font-family:var(--font-serif);font-size:1.1rem;margin-bottom:8px}.consult-card p{font-size:.83rem;color:#94A3B8;margin-bottom:16px}.wa-btn{display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#fff;padding:11px 22px;border-radius:100px;font-weight:600;font-size:.88rem;width:100%;justify-content:center}.price-badge{background:linear-gradient(135deg,#14B8A6,#0D9488);color:#fff;border-radius:12px;padding:18px;text-align:center;margin-bottom:12px}.price-badge .amount{font-family:var(--font-serif);font-size:1.8rem;font-weight:700}.price-badge .from{font-size:.78rem;opacity:.8}.service-list li{padding:7px 0;border-bottom:1px solid #E2E8F0;font-size:.86rem;display:flex;align-items:center;gap:8px}.service-list li:last-child{border-bottom:none}.service-list li i{color:#14B8A6;width:14px}@media(max-width:900px){.blog-layout{grid-template-columns:1fr}.nav-links,.nav-phone{display:none}.hamburger{display:flex}}@media(max-width:600px){.article-body{padding:24px 20px}.blog-hero{padding:44px 0 36px}}</style>`;

const NAV = `<div class="topbar"><div class="container"><div class="topbar-promo-wrap"><span class="topbar-promo">KAMBOH ASSOCIATES LAHORE — ALLAMA IQBAL TOWN, OPP. SHELL PUMP, MAIN BOULEVARD &nbsp;&bull;&nbsp; 0328-4675162 &nbsp;&bull;&nbsp; TAX CONSULTANT LAHORE &nbsp;&bull;&nbsp; 18+ YEARS EXPERIENCE &nbsp;&bull;&nbsp;</span></div></div></div>
<nav class="navbar"><div class="container"><a href="/" class="logo"><div><svg width="44" height="44" viewBox="0 0 46 46" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="g1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#1a3a1a"/><stop offset="100%" stop-color="#0d2a0d"/></linearGradient><linearGradient id="g2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#F0CF6B"/><stop offset="100%" stop-color="#D4AF37"/></linearGradient></defs><circle cx="23" cy="23" r="23" fill="url(#g1)"/><circle cx="23" cy="23" r="21" fill="none" stroke="url(#g2)" stroke-width="1.5"/><text x="23" y="27" font-family="Cinzel,serif" font-size="13" font-weight="700" fill="url(#g2)" text-anchor="middle" letter-spacing="1">KA</text><path id="cp" d="M 23,23 m -17,0 a 17,17 0 1,1 34,0 a 17,17 0 1,1 -34,0" fill="none"/><text font-family="Cinzel,serif" font-size="3.8" fill="#D4AF37" letter-spacing="1.8"><textPath href="#cp" startOffset="3%">KAMBOH ASSOCIATES &bull; TAX CONSULTANT &bull;</textPath></text></svg></div><div><div class="logo-name">KAMBOH ASSOCIATES</div><div class="logo-sub">Tax &amp; Regulatory Excellence</div></div></a>
<ul class="nav-links"><li><a href="/">Home</a></li><li><a href="/income-tax-filing">Services <i class="fas fa-chevron-down nav-chevron"></i></a><div class="nav-drop"><a href="/income-tax-filing"><i class="fas fa-file-invoice-dollar"></i> Income Tax Filing</a><a href="/ntn-registration"><i class="fas fa-id-card"></i> NTN Registration</a><a href="/sales-tax-registration"><i class="fas fa-percent"></i> Sales Tax Registration</a><a href="/secp-vs-sole-proprietorship-pakistan"><i class="fas fa-building"></i> Company Registration</a><div class="nav-drop-divider"></div><a href="/fbr-notice-defense"><i class="fas fa-shield-alt"></i> FBR Notice Defense</a><a href="/wealth-statement"><i class="fas fa-wallet"></i> Wealth Statement</a><a href="/withholding-tax"><i class="fas fa-hand-holding-usd"></i> Withholding Tax</a><a href="/aop-tax-return"><i class="fas fa-handshake"></i> AOP Tax Return</a><div class="nav-drop-divider"></div><a href="/pricing"><i class="fas fa-tags"></i> Pricing &amp; Fees</a></div></li><li><a href="/pricing">Pricing</a></li><li><a href="/contact">Contact</a></li><li><a href="/blogs">Blog</a></li></ul>
<div class="nav-actions"><span class="nav-phone"><i class="fas fa-phone"></i><a href="tel:03284675162">0328-4675162</a></span><a href="/contact" class="nav-cta">Get Started</a><button class="hamburger" id="hamburger" onclick="this.classList.toggle('open');document.getElementById('mobileNav').classList.toggle('open')" aria-label="Menu"><span></span><span></span><span></span></button></div></div></nav>
<div class="mobile-nav" id="mobileNav"><a href="/">Home</a><a href="/income-tax-filing">Services</a><a href="/pricing">Pricing</a><a href="/blogs">Blog</a><a href="/contact">Contact</a><a href="https://wa.me/923284675162" target="_blank" style="color:#25D366"><i class="fab fa-whatsapp" style="margin-right:8px"></i>WhatsApp</a></div>`;

const SIDEBAR = `<aside><div class="consult-card"><h4>Free Consultation</h4><p>Expert reply within 30 minutes</p><a href="https://wa.me/923284675162?text=I+need+tax+help+in+Lahore" class="wa-btn"><i class="fab fa-whatsapp"></i> WhatsApp Now</a></div><div class="sidebar-card"><h4>Our Services</h4><ul class="service-list"><li><i class="fas fa-file-invoice-dollar"></i><a href="/income-tax-filing" style="color:#0D9488"> Income Tax Return</a></li><li><i class="fas fa-id-card"></i><a href="/ntn-registration" style="color:#0D9488"> NTN Registration</a></li><li><i class="fas fa-building"></i><a href="/secp-vs-sole-proprietorship-pakistan" style="color:#0D9488"> Company Registration</a></li><li><i class="fas fa-percent"></i><a href="/sales-tax-registration" style="color:#0D9488"> Sales Tax (STRN)</a></li><li><i class="fas fa-shield-halved"></i><a href="/fbr-notice-defense" style="color:#0D9488"> FBR Notice Defense</a></li><li><i class="fas fa-wallet"></i><a href="/wealth-statement" style="color:#0D9488"> Wealth Statement</a></li></ul></div><div class="sidebar-card"><div class="price-badge"><div class="from">Starting From</div><div class="amount">Rs. 3,500</div><div class="from">Income Tax Return</div></div><a href="/pricing" style="display:block;text-align:center;color:#14B8A6;font-size:.85rem;font-weight:600;margin-top:8px"><i class="fas fa-tags"></i> View All Pricing</a></div><div class="sidebar-card" style="background:#FFF7ED;border-color:#FED7AA"><h4 style="color:#9A3412">📍 Our Office</h4><p style="font-size:.82rem;color:#7C3306;line-height:1.6">Allama Iqbal Town<br>Opp. Shell Pump<br>Main Boulevard, Lahore</p><a href="tel:03284675162" style="display:block;margin-top:10px;color:#0D9488;font-weight:600;font-size:.85rem"><i class="fas fa-phone"></i> 0328-4675162</a></div></aside>`;

const FOOTER = `<a href="https://wa.me/923284675162" target="_blank" aria-label="WhatsApp" style="position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;justify-content:center;width:58px;height:58px;background:#25D366;border-radius:50%;box-shadow:0 4px 16px rgba(37,211,102,0.5)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="30" height="30" fill="#fff"><path d="M16 0C7.163 0 0 7.163 0 16c0 2.822.736 5.476 2.027 7.785L0 32l8.469-2.001A15.93 15.93 0 0016 32c8.837 0 16-7.163 16-16S24.837 0 16 0zm0 29.333a13.27 13.27 0 01-6.77-1.855l-.485-.288-5.024 1.188 1.228-4.893-.317-.502A13.267 13.267 0 012.667 16C2.667 8.636 8.636 2.667 16 2.667S29.333 8.636 29.333 16 23.364 29.333 16 29.333zm7.27-9.862c-.398-.199-2.354-1.162-2.72-1.294-.365-.133-.631-.199-.897.199-.265.398-1.029 1.294-1.261 1.56-.232.265-.465.299-.863.1-.398-.199-1.681-.619-3.202-1.977-1.183-1.057-1.981-2.362-2.213-2.76-.232-.398-.025-.613.175-.811.179-.179.398-.465.597-.698.199-.232.265-.398.398-.664.133-.265.066-.498-.033-.697-.1-.199-.897-2.163-1.229-2.96-.324-.777-.653-.672-.897-.684l-.764-.013c-.265 0-.697.1-1.062.498-.365.398-1.394 1.362-1.394 3.322s1.428 3.853 1.627 4.119c.199.265 2.808 4.287 6.804 6.014.951.411 1.693.656 2.271.84.954.304 1.823.261 2.509.158.765-.114 2.354-.962 2.686-1.891.333-.929.333-1.726.233-1.891-.1-.165-.365-.265-.764-.464z"/></svg></a>
<footer style="background:#0F172A;border-top:1px solid rgba(255,255,255,.06);padding:28px 0;text-align:center"><p style="font-size:.82rem;color:#64748B">&#169; 2026 Kamboh Associates — Tax Consultant Lahore &nbsp;|&nbsp;<a href="/contact" style="color:#14B8A6">Contact</a> &nbsp;|&nbsp;<a href="/pricing" style="color:#14B8A6">Pricing</a> &nbsp;|&nbsp;<a href="/blogs" style="color:#14B8A6">Blog</a></p></footer>
<script>document.querySelectorAll('.faq-q').forEach(q=>q.addEventListener('click',()=>q.parentElement.classList.toggle('open')))</script>`;

function generatePage({ slug, area, city, province }) {
  const areaLabel = `${area}, Lahore`;
  const canonical = `https://kambohassociates.com/blogs/${slug}`;
  const waMsg = encodeURIComponent(`I need a tax consultant in ${area}, Lahore. Please help.`);

  const schema = JSON.stringify({"@context":"https://schema.org","@type":"BlogPosting","headline":`Best Tax Consultant in ${areaLabel} 2026 — FBR Filing, NTN Registration`,"datePublished":"2026-06-23","dateModified":"2026-06-23","author":{"@type":"Organization","name":"Kamboh Associates","url":"https://kambohassociates.com"},"publisher":{"@type":"Organization","name":"Kamboh Associates","url":"https://kambohassociates.com","logo":{"@type":"ImageObject","url":"https://kambohassociates.com/logo.png"}},"mainEntityOfPage":{"@type":"WebPage","@id":canonical}});
  const faqSchema = JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":`Who is the best tax consultant in ${areaLabel}?`,"acceptedAnswer":{"@type":"Answer","text":`Kamboh Associates is a top-rated FBR-registered tax consultancy based in Lahore (Allama Iqbal Town, Opp. Shell Pump, Main Boulevard). We serve clients in ${areaLabel} and all of Lahore — 100% online via WhatsApp 0328-4675162. Services: income tax filing, NTN, company registration, FBR notice defense.`}},{"@type":"Question","name":`Is there a tax consultant near me in ${area} Lahore?`,"acceptedAnswer":{"@type":"Answer","text":`Yes — Kamboh Associates serves ${areaLabel} clients online. WhatsApp documents to 0328-4675162, return filed same day. No office visit needed.`}},{"@type":"Question","name":`What is the fee for income tax return in ${area} Lahore?`,"acceptedAnswer":{"@type":"Answer","text":`Income tax return filing in Lahore starts from Rs. 3,500 for salaried individuals. Business/freelancer returns from Rs. 5,000. View full pricing at kambohassociates.com/pricing.`}}]});

  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Best Tax Consultant in ${areaLabel} 2026 — FBR Filing, NTN &amp; Company Registration</title>
<meta name="description" content="Looking for a tax consultant in ${areaLabel}? Kamboh Associates — FBR registered, 18+ years, Lahore based. Income tax return Rs. 3,500. NTN same day. 100% online. WhatsApp 0328-4675162."/>
<meta name="keywords" content="tax consultant ${area.toLowerCase()} Lahore, best tax consultant ${area.toLowerCase()}, tax consultant near me ${area.toLowerCase()}, FBR filing ${area.toLowerCase()} Lahore, NTN registration ${area.toLowerCase()} Lahore, income tax return ${area.toLowerCase()}"/>
<link rel="canonical" href="${canonical}"/>
<link rel="preconnect" href="https://fonts.googleapis.com"/>
<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700&family=DM+Sans:wght@400;500;600&family=Cinzel:wght@600;700&display=swap" rel="stylesheet"/>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
${SHARED_STYLE}
<script type="application/ld+json">${schema}</script>
<script type="application/ld+json">${faqSchema}</script>
<meta property="og:locale" content="en_PK"/><meta property="og:type" content="article"/>
<meta property="og:title" content="Best Tax Consultant in ${areaLabel} 2026 — Kamboh Associates"/>
<meta property="og:description" content="Tax return filing, NTN registration, company registration in ${areaLabel}. FBR registered. Rs. 3,500 starting fee. WhatsApp 0328-4675162."/>
<meta property="og:url" content="${canonical}"/>
</head><body>
${NAV}
<section class="blog-hero"><div class="container">
<nav class="breadcrumb"><a href="/"><i class="fas fa-home"></i> Home</a><i class="fas fa-chevron-right"></i><a href="/blogs">Blog</a><i class="fas fa-chevron-right"></i><a href="/blogs/tax-consultant-lahore">Tax Consultant Lahore</a><i class="fas fa-chevron-right"></i><span>${area}</span></nav>
<div class="blog-cat"><i class="fas fa-map-marker-alt"></i> Lahore — Local Tax Guide</div>
<h1>Best Tax Consultant in ${areaLabel} — Income Tax, NTN &amp; FBR Filing 2026</h1>
<div class="blog-meta"><span><i class="fas fa-calendar"></i> June 2026</span><span><i class="fas fa-user"></i> Kamboh Associates</span><span><i class="fas fa-map-marker-alt"></i> ${area}, Lahore</span></div>
</div></section>

<div class="blog-layout container"><article class="article-body">
<p class="article-lead">Searching for a reliable tax consultant in <strong>${areaLabel}</strong>? Kamboh Associates is Lahore's trusted FBR-registered tax consultancy — located at Allama Iqbal Town, Opp. Shell Pump, Main Boulevard. We serve clients across all of Lahore including ${area}, 100% online via WhatsApp. File your income tax return, get NTN, or handle an FBR notice — same day, from home.</p>

<h2>Tax Services in ${areaLabel} — Fees &amp; Timeline</h2>
<table class="data-table">
<tr><th>Service</th><th>Starting Fee</th><th>Delivery</th></tr>
<tr><td>Salaried Individual Tax Return</td><td>Rs. 3,500</td><td>Same day</td></tr>
<tr><td>Business / Freelancer Tax Return</td><td>Rs. 5,000</td><td>1–2 days</td></tr>
<tr><td>NTN Registration</td><td>Rs. 2,000</td><td>Same day</td></tr>
<tr><td>Sales Tax Registration (STRN)</td><td>Rs. 3,000</td><td>2–3 days</td></tr>
<tr><td>Company Registration (SECP Pvt Ltd)</td><td>Rs. 15,000</td><td>7–10 days</td></tr>
<tr><td>FBR Notice Response</td><td>Rs. 5,000</td><td>1–3 days</td></tr>
<tr><td>Wealth Statement</td><td>Rs. 2,500</td><td>Same day</td></tr>
<tr><td>AOP / Partnership Return</td><td>Rs. 8,000</td><td>2–3 days</td></tr>
</table>

<h2>Why Lahore Residents in ${area} Choose Kamboh Associates</h2>
<ul>
<li><strong>Based in Lahore since 2008</strong> — 18+ years serving Punjab clients</li>
<li><strong>FBR registered</strong> — authorized to file on your behalf on IRIS</li>
<li><strong>5,000+ Lahore clients</strong> — salaried employees, business owners, freelancers</li>
<li><strong>100% online via WhatsApp</strong> — no need to come to our office</li>
<li><strong>Same-day filing</strong> for salaried returns — receive FBR receipt same day</li>
<li><strong>4.9★ rated</strong> — consistent reviews from Lahore clients</li>
</ul>
<div class="highlight-box"><p><strong>How to file from ${area}:</strong> WhatsApp your CNIC + salary certificate (or bank statement) to <strong>0328-4675162</strong>. We reply in 30 minutes, file your return, and send your FBR acknowledgment — all same day.</p></div>

<h2>What Documents Do You Need for Tax Filing in Lahore?</h2>
<table class="data-table">
<tr><th>Your Situation</th><th>Documents Required</th></tr>
<tr><td>Salaried person</td><td>Annual salary certificate from employer + CNIC</td></tr>
<tr><td>Freelancer / IT exporter</td><td>Bank statement (Wise/Payoneer/HBL etc.) + CNIC</td></tr>
<tr><td>Business owner</td><td>Revenue &amp; expense summary or bank statement</td></tr>
<tr><td>Property owner / landlord</td><td>Property docs, rent agreement, bank statement</td></tr>
<tr><td>All filers</td><td>List of all assets (property, car, cash) + loans/liabilities</td></tr>
</table>

<h2>FBR Notice? We Respond for ${area} Residents</h2>
<p>FBR sends notices to Lahore taxpayers for undeclared income, property purchases, and bank transactions. Do not ignore any notice — respond within the deadline stated in the notice. Common notices:</p>
<ul>
<li><strong>Section 111:</strong> Unexplained wealth or income — most common in Lahore</li>
<li><strong>Section 122:</strong> Assessment amendment</li>
<li><strong>Section 176:</strong> Requirement to furnish information or records</li>
</ul>
<div class="highlight-box"><p>Received a notice? WhatsApp a photo to <strong>0328-4675162</strong> right now. Free initial assessment — we tell you exactly what to do within 30 minutes.</p></div>

<h2>Frequently Asked Questions</h2>
<div class="faq-list">
<div class="faq-item"><div class="faq-q">Who is the best tax consultant in ${areaLabel}? <i class="fas fa-chevron-down"></i></div><div class="faq-a">Kamboh Associates — located at Allama Iqbal Town, Main Boulevard Lahore — is one of Lahore's most trusted FBR-registered tax consultancies. With 18+ years of experience and 5,000+ clients, we serve ${area} and all of Lahore online via WhatsApp at 0328-4675162.</div></div>
<div class="faq-item"><div class="faq-q">Is there a tax consultant near me in ${area}? <i class="fas fa-chevron-down"></i></div><div class="faq-a">Yes — Kamboh Associates serves ${area} Lahore 100% online. No need to visit an office. WhatsApp documents to 0328-4675162 and your tax return is filed and receipted the same day.</div></div>
<div class="faq-item"><div class="faq-q">How much does tax filing cost in ${area}, Lahore? <i class="fas fa-chevron-down"></i></div><div class="faq-a">Salaried individual returns start from Rs. 3,500. Business/freelancer returns from Rs. 5,000. NTN registration Rs. 2,000. All fees are fixed and confirmed before work starts. Full fee list at kambohassociates.com/pricing.</div></div>
<div class="faq-item"><div class="faq-q">Can you file my return without me visiting your Lahore office? <i class="fas fa-chevron-down"></i></div><div class="faq-a">Absolutely. Our entire process is online. You WhatsApp your documents from ${area}, we file on IRIS, and you get the FBR acknowledgment receipt on WhatsApp — same day. Thousands of Lahore clients file this way every year.</div></div>
</div>

<div class="article-cta" style="background:linear-gradient(135deg,#0F172A,#1E3A5F);border-radius:16px;padding:28px 32px;margin-top:32px;text-align:center">
<h3 style="font-family:'Cormorant Garamond',serif;color:#fff;font-size:1.4rem;margin-bottom:8px">Tax Consultant ${area}, Lahore — WhatsApp Now</h3>
<p style="color:#94A3B8;font-size:.9rem;margin-bottom:18px">Send documents → Return filed same day → FBR receipt on WhatsApp. Simple &amp; trusted.</p>
<a href="https://wa.me/923284675162?text=${waMsg}" style="display:inline-flex;align-items:center;gap:8px;background:#25D366;color:#fff;padding:12px 28px;border-radius:100px;font-weight:600;font-size:.92rem"><i class="fab fa-whatsapp"></i> WhatsApp 0328-4675162</a>
</div>
</article>
${SIDEBAR}
</div>
${FOOTER}
</body></html>`;
}

let created = 0;
const newSlugs = [];
for (const loc of LOCATIONS) {
  const fp = path.join(BLOGS_DIR, `${loc.slug}.html`);
  if (fs.existsSync(fp)) { process.stdout.write('.'); continue; }
  fs.writeFileSync(fp, generatePage(loc), 'utf8');
  newSlugs.push(loc.slug);
  created++;
  if (created % 20 === 0) process.stdout.write(`[${created}]`);
}
console.log(`\n✅ Created ${created} new Lahore pages`);

if (newSlugs.length > 0) {
  const blogsPath = path.join(__dirname, 'blogs.html');
  let b = fs.readFileSync(blogsPath, 'utf8');
  const links = newSlugs.map(s => {
    const loc = LOCATIONS.find(l => l.slug === s);
    return `      <a href="/blogs/${s}" style="display:block;padding:8px 0;font-size:.83rem;color:#334155;border-bottom:1px solid #E2E8F0;transition:color .15s" onmouseover="this.style.color='#14B8A6'" onmouseout="this.style.color='#334155'">Tax Consultant ${loc.area}, Lahore</a>`;
  }).join('\n');
  b = b.replace('      <a href="/blogs/income-tax-slab-pakistan-2026"', links + '\n      <a href="/blogs/income-tax-slab-pakistan-2026"');
  fs.writeFileSync(blogsPath, b, 'utf8');
  console.log(`✅ Added ${newSlugs.length} links to blogs.html`);
}
